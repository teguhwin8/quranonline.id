...
export default function GlobalAudioPlayer() {
  ...
  const { surah, ayahs, currentAyahIndex, isPlaying, autoPlayNextSurah } =
    audioState;
  const currentAyah =
    currentAyahIndex !== null ? ayahs[currentAyahIndex] : null;
  ...
  const playNext = useCallback(async () => {
    if (
      ...
      try {
        ...
        const { surah: nextSurah, ayahs: nextAyahs } = await getSurahComplete(
          nextSurahNumber,
          selectedReciter,
        );
  ...
  // Sync progress bar width from shared audioProgress context
  useEffect(() => {
  ...
  // Sync currentTime display using audio element events (not isLoaded dependency)
  useEffect(() => {
    ...
    audio.addEventListener("play", startTimeLoop);
    audio.addEventListener("pause", stopTimeLoop);
    audio.addEventListener("ended", stopTimeLoop);

    return () => {
      audio.removeEventListener("play", startTimeLoop);
      audio.removeEventListener("pause", stopTimeLoop);
      audio.removeEventListener("ended", stopTimeLoop);
      stopTimeLoop();
    };
  }, [audioRef]);

  // Track previous ayah for detecting changes
  const prevAyahRef = useRef<{
  ...
  useEffect(() => {
    ...
    const ayahChanged =
      prevAyahRef.current.surahNum !== currentSurahNum ||
  ...
  useEffect(() => {
    ...
    const handleError = () => {
      ...
      if (
        currentAyah?.audioSecondary &&
        currentAyah.audioSecondary.length > 0
      ) {
        const fallbackUrl = currentAyah.audioSecondary[0];
        console.warn(
          "Primary audio failed, trying audioSecondary:",
          fallbackUrl,
        );
  ...
  const handleNavigateToAyah = () => {
    if (surah && currentAyah) {
      ...
      if (surah.number < 0) {
        ...
        router.push(
          `/juz/${juzNumber}#ayah-${ayahSurahNum}-${currentAyah.numberInSurah}`,
        );
      } else {
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={playNext}
                  disabled={
                    currentAyahIndex === ayahs.length - 1 && !autoPlayNextSurah
                  }
                  className="icon-btn disabled:opacity-30"
              ...
              <button
                ...
                aria-label={
                  autoPlayNextSurah ? "Autoplay aktif" : "Autoplay nonaktif"
                }
                title={
                  autoPlayNextSurah
                    ? "Autoplay: ON (Lanjut ke surat berikutnya)"
                    : "Autoplay: OFF"
                }
              >
