...
export default function JuzClientWrapper({ juzNumber, ayahs: initialAyahs }: JuzClientWrapperProps) {
    ...
    }, [audioState.surah?.number, audioState.currentAyahIndex, audioState.isPlaying, juzNumber, playWithGesture, pauseAudio, resumeAudio, ayahs]);
    ...
    const handlePlayAll = useCallback(() => {
        ...
        const needsBismillah = startsAtAyah1 && firstSurahNumber !== 1 && firstSurahNumber !== 9;
    ...
    const hasBismillahPrepended = isCurrentJuz && audioState.ayahs.length > 0 &&
        audioState.ayahs[0]?.numberInSurah === 0;
    ...
    const createBookmarkHandler = useCallback((ayah: JuzAyahWithTranslation) => {
    ...
    const ayahStates = useMemo(() => {
        return ayahs.map((ayah, index) => {
            ...
            return {
                isPlaying: isCurrentJuz && currentPlayingIndex === audioIndex && isAudioPlaying,
                isBookmarked: isBookmarked(ayah.surah.number, ayah.numberInSurah),
            };
        });
    }, [ayahs, isCurrentJuz, currentPlayingIndex, isAudioPlaying, isBookmarked, hasBismillahPrepended]);
            ...
            {viewMode === 'per-ayat' ? (
                ...
                <div className="space-y-8">
                    {surahGroups.map((group, groupIndex) => {
                        ...
                        const showBismillah = groupIndex > 0 && group.ayahs[0]?.numberInSurah === 1 && group.surah.number !== 9 && group.surah.number !== 1;
                                ...
                                <div className="surah-header-card mb-4">
                                    <div className="flex items-center justify-between gap-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <Link href={`/surah/${group.surah.number}`} className="font-semibold text-foreground hover:text-primary transition-colors">
                                        ...
                                        <div className="text-right text-sm text-foreground-muted">
                                            Ayat {group.ayahs[0].numberInSurah}–{group.ayahs[group.ayahs.length - 1].numberInSurah}
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        return (
                                            ...
                                            >
                                                <AyahCard
                                                    ...
                                                    surahNumber={ayah.surah.number}
                                                    surahName={ayah.surah.englishName}
                                                    isPlaying={ayahStates[globalIndex]?.isPlaying || false}
                                                    isBookmarked={ayahStates[globalIndex]?.isBookmarked || false}
