...
export default function SurahClientWrapper({
  surah,
  ayahs: initialAyahs,
}: SurahClientWrapperProps) {
  const {
    audioState,
    audioProgress,
    playWithGesture,
  ...
  const ayahStates = useMemo(() => {
    ...
    return ayahs.map((ayah, index) => ({
      isPlaying:
        isCurrentSurah &&
        currentPlayingIndex === index + indexOffset &&
        isAudioPlaying,
  ...
  // Compute the page index of the currently playing ayah (accounting for bismillah offset)
  const playingPageIndex = useMemo(() => {
    if (!isCurrentSurah || currentPlayingIndex === null) return null;
    const hasBismillahPrepended =
      audioState.ayahs.length > 0 && audioState.ayahs[0]?.numberInSurah === 0;
    const indexOffset = hasBismillahPrepended ? 1 : 0;
    const pageIdx = currentPlayingIndex - indexOffset;
    return pageIdx >= 0 ? pageIdx : null;
  }, [isCurrentSurah, currentPlayingIndex, audioState.ayahs]);
  ...
  return (
    <>
      ...
      {viewMode === "per-ayat" ? (
          ...
          {ayahs.map((ayah, index) => (
            ...
            >
              <AyahCard
                ...
                playbackProgress={
                  ayahStates[index].isPlaying ? audioProgress : undefined
                }
        ...
        /* Per-Surat View: Continuous Arabic text with character-by-character karaoke */
        <div className="card fade-in">
          <p className="continuous-arabic-view font-arabic" dir="rtl">
            {ayahs.map((ayah, index) => {
              const isActiveAyah = playingPageIndex === index && isAudioPlaying;
